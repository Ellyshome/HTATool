...
// 辅助函数
// -------------------------
//这里曾经是function deepCloneStyle(s) 
// ---------- 用这个函数替换原来的 deepCloneStyle ----------
function deepCloneStyle(source) {
if (!source) return {};
...
for (let key in color) {
    // 保留所有颜色属性，包括rgb、theme、indexed、tint等
    clonedColor[key] = color[key];
}
return clonedColor;
}
...
if (source.font) {
target.font = {};
for (let key in source.font) {
    if (key === 'color') {
        target.font.color = cloneColor(source.font.color);
    } else {
        target.font[key] = source.font[key];
    }
}
}

// 2. 复制填充样式 - 确保patternType和所有颜色正确
if (source.fill) {
target.fill = {};

// 复制patternType（确保不丢失填充模式）
if (source.fill.patternType !== undefined) {
    target.fill.patternType = source.fill.patternType;
}

// 复制前景色和背景色
if (source.fill.fgColor) {
    target.fill.fgColor = cloneColor(source.fill.fgColor);
}
if (source.fill.bgColor) {
    target.fill.bgColor = cloneColor(source.fill.bgColor);
}

// 复制可能的其他填充属性
for (let key in source.fill) {
    if (!['fgColor', 'bgColor', 'patternType'].includes(key)) {
        target.fill[key] = source.fill[key];
    }
}
}

// 3. 复制边框样式 - 完整复制所有边框和颜色
if (source.border) {
target.border = {};
for (let key in source.border) {
    const borderSide = source.border[key];
    if (borderSide && typeof borderSide === 'object') {
        target.border[key] = {};
        for (let borderKey in borderSide) {
            if (borderKey === 'color') {
                target.border[key].color = cloneColor(borderSide.color);
            } else {
                target.border[key][borderKey] = borderSide[borderKey];
            }
        }
    }
}
...
if (!['font', 'fill', 'border', 'alignment', 'numFmt'].includes(key)) {
    // 避免重复复制
...
// XLSX 地址 helper: encode_cell / decode_range 已由 xlsx 库提供
...
class Doctor {
    ...
    constructor(cell, sheetTitle, r, c) {
        ...
        if (this.name.length > 4 || this.name.includes('皮')) {
            console.warn(`在表<${sheetTitle}>发现疑似非法姓名： <${this.name}> , 丢弃`);
            this.section = '错误';
        }
    }
...
function getDoctors(sheet) {
    ...
    // 读取 A3 的颜色 (A3 -> row index 2, col 0)
    ...
    for (let r = 0; r <= range.e.r; r++) {
        ...
        // 优先用颜色匹配：如果 A3 存在字体颜色，则把与 A3 同色且文本短的列为医生
        if (baseColor) {
    ...
    matched.forEach(doc => {
        ...
        for (let day = 1; day <= 14; day++) {
            ...
            if (flag === 0) {
                ...
                const vs = cleanCompareVal(subCell.v);
                const vm = cleanCompareVal(masterCell.v);
                if (vs !== vm) {
...
function delflag(sheet) {
    ...
    for (let c = 2; c <= 15 && c <= range.e.c; c++) {
        for (let r = 1; r <= range.e.r; r++) { // Python 从 row 2 开始 -> 0-based 从1开始
            ...
            sheet[addrAm].v = am;
            if (cell.s) sheet[addrAm].s = deepCloneStyle(cell.s);
            // PM
            if (!sheet[addrPm]) sheet[addrPm] = {};
            sheet[addrPm].v = pm;
            if (cell.s) sheet[addrPm].s = deepCloneStyle(cell.s);
        }
    }
}
...
function downloadResult() {
    ...
    try {
        ...
        const writeOptions = {
            ...
            bookSST: false,
            type: 'file',
            cellStyles: true,   // 确保所有单元格样式被保留
            cellNF: true,       // 确保数字格式被保留
            cellDates: true,    // 确保日期格式被保留
            sheetStubs: true    // 确保空单元格样式被保留
        };
